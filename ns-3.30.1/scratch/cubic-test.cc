...
bool newSsThresFile = true;
bool newCubicWMaxFile = true;
bool newConcaveConvexFile = true;
...
/**
 * Callback method to log changes to the cubic w_max.
 */
static void
CubicWMaxChange (uint32_t oldWMax, uint32_t newWMax)
{
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t" << newWMax);

  // Write to a file
  ofstream myfile;
  if (newCubicWMaxFile)
    {
      myfile.open ("cubic-w_max.log");
      newCubicWMaxFile = false;
    }
  else
    {
       myfile.open ("cubic-w_max.log", ios::out | ios::app);
    }
     myfile << Simulator::Now ().GetSeconds () << "," << newWMax << "\n"; 
     myfile.close();
}
...
static void
ConcaveConvexChange (bool oldBelowOrigin, bool newBelowOrigin)
{
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t" << newBelowOrigin);

  // Write to a file
  ofstream myfile;
  if (newConcaveConvexFile)
    {
      myfile.open ("cubic-concave-convex.log");
      newConcaveConvexFile = false;
    }
  else
    {
       myfile.open ("concave-convex.log", ios::out | ios::app);
    }
     myfile << Simulator::Now ().GetSeconds () << "," << newBelowOrigin << "\n"; 
     myfile.close();
}
...
main (int argc, char *argv[])
{
  ...
  // Trace changes to the congestion window and slow start threshold.
  Config::ConnectWithoutContext ("/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/CongestionWindow", MakeCallback (&CwndChange));
  Config::ConnectWithoutContext ("/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/SlowStartThreshold", MakeCallback (&SsThreshChange));
  Config::ConnectWithoutContext ("/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/CongestionWindowMax", MakeCallback (&CubicWMaxChange));
  Config::ConnectWithoutContext ("/NodeList/0/$ns3::TcpL4Protocol/SocketList/0/ConcaveConvexChange", MakeCallback (&ConcaveConvexChange));
